...
export const Dashboard = () => {
  const [stakedTokens, setStakedTokens] = useState([
    "0x62f9D3A871cd539EEF606cF3cdF77c30386128AB",
  ]);
          ...
          return (
            <Link to={`/dashboard/${stakedTokens[i]}`}>
              <div className="p-4 aspect-square group cursor-pointer relative flex rounded-2xl bg-zinc-50 transition-shadow hover:shadow-md hover:shadow-zinc-900/5">
                ...
                <div className=" ">
                  <p className="text-sm font-semibold leading-7 text-zinc-900 uppercase">
                    {tokenSymbols[i]}
                  </p>
                  {/* <h1 className="mt-4 text-zinc-900 font-semibold">{e}</h1> */}
                  <h1 className=" text-zinc-300 ">{`${stakedTokens[i].slice(
                    0,
                    4
                  )}...${stakedTokens[i].slice(-4)}`}</h1>
                  {/* <p>{ tokenBalance[i]}</p> */}
                </div>
              </div>
            </Link>
